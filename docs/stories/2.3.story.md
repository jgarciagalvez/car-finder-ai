...
5. AI prompts are organized as versioned markdown files following BMAD-style agent patterns, not hardcoded in TypeScript.
6. A separate script (`apps/api/src/scripts/analyze.ts`) is created to run analysis on all un-analyzed vehicles.

## Tasks / Subtasks
- [ ] Design prompt file structure and organization (AC: 5)
...
- [ ] Create prompt definition files (AC: 1, 2, 3, 4, 5)
...
- [ ] Create AIService in backend API package (AC: 1, 2, 3, 4, 5)
...
- [ ] Implement Personal Fit Score generation (AC: 1, 5)
...
- [ ] Implement AI Priority Rating and Summary (AC: 2, 5)
...
- [ ] Implement Virtual Mechanic's Report (AC: 3, 5)
...
- [ ] Implement Data Sanity Check (AC: 4, 5)
...
- [ ] Create analysis script (AC: 6)
  - [ ] Create `apps/api/src/scripts/analyze.ts` file
  - [ ] Import AIService and VehicleRepository
  - [ ] Implement query to find vehicles with NULL AI fields
  - [ ] Implement batch processing loop with rate limit respect (15 RPM)
  - [ ] Call AIService methods for each un-analyzed vehicle
  - [ ] Update vehicle records in database with AI results
...
async findVehiclesWithoutAnalysis(): Promise<Vehicle[]> {
  return await db
    .selectFrom('vehicles')
    .selectAll()
    .where('personalFitScore', 'is', null)
...
- Test script identifies vehicles with NULL AI fields correctly
- Test script processes vehicles in batches respecting rate limits
- Test script updates database with AI analysis results
...
- Test `findVehiclesWithoutAnalysis()` query returns correct vehicles
- Test `updateVehicleAnalysis()` updates AI fields correctly
...
- Manually test prompts with real Gemini API to verify output quality
- Validate prompt responses match expected format
- Iterate on prompt wording if LLM responses are inconsistent
- Ensure all 4 prompt files follow the standard template format
